...
router.post('/signin', function (req, res) {
    ...
    connAttrs.query("SELECT password, org_id, email, id, username, role, DATE_FORMAT(dade_date, '%Y-%m-%d') dade_date FROM p_users where email=? AND deleted_yn ='N' AND (dade_date - curdate()) > 0", user1.email, function (error, result) {
        if (error || result < 1) {
...
router.post('/ExpiredItems', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        var sql = "SELECT * from vw_items_expired WHERE expire_date < curdate() order by expire_date ASC";
        connAttrs.query(sql, function (error, results) {
            if (error || results.length < 1) {
                res.set('Content-Type', 'application/json');
                var status = error ? 500 : 404;
...
// items due to expire in 2 months

router.post('/dueExpiry', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        connAttrs.query(sql, function (error, results) {
            if (error || results.length < 1) {
                ...
                res.status(status).send(JSON.stringify({
                    status: status,
                    message: error ? "Error getting the server" : "No Items or products found due to Expire",
                    detailed_message: error ? error.message : "Sorry there are no Products due to Eapire. Please Keep Checking"
                }));
                return (error);
            }
...
router.post('/checkOut', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        if (check_out.sell_date != null) {
            // when backdating the sells
            connAttrs.query(sql, {
                ...
                created_date: `${check_out.sell_date} 18:00:00`,
            ...
            connAttrs.query(sql, {
                ...
                quantity_to: check_out.quantity_to,
                customer_contact: check_out.contact,
                // created_date: check_out.sell_date,
                payment_mode: check_out.payment_mode,
                sold_amount: (check_out.quantity_from - check_out.quantity_to) * check_out.item_price
            }, function (error, result) {
                if (error) {
...
// Disposing expired Products logs
router.post('/dispose', function (req, res) {
    var dispose_out = {
        category_id: req.body.category_id,
        item_id: req.body.item_id,
        quantity_from: req.body.quantity_from,
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        var sql = "INSERT INTO p_logs SET?";
        connAttrs.query(sql, {
            category_id: dispose_out.category_id,
            item_id: dispose_out.item_id,
            created_by: decoded.entity_id,
            log_name: 'Dispose Out',
            quantity_from: dispose_out.quantity_from,
            quantity_to: dispose_out.quantity_to
            // cost_incured: dispose_out.quantity_from * dispose_out.buying_price
        }, function (error, result) {
            if (error) {
                res.set('Content-Type', 'application/json');
                var status = 500;
...
router.post('/suppliersReport', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        var sql = "SELECT * FROM pharmacy.VW_SUPPLIER_REPORT WHERE created_date <= LAST_DAY(curdate()) AND created_date >= date_add(date_add(LAST_DAY(curdate()),interval 1 DAY),interval -1 MONTH) ORDER BY created_date DESC";
        connAttrs.query(sql, function (error, results) {
            if (error || results.length < 1) {
                res.set('Content-Type', 'application/json');
                var status = error ? 500 : 404;
                res.status(status).send(JSON.stringify({
                    status: status,
                    message: error ? "Error getting the server" : "No Records found for Supplier this month",
                    detailed_message: error ? error.message : "Sorry there are no Records Found for monthly supplier details."
                }));
                return (error);
            }

            res.contentType('application/json').status(200).send(JSON.stringify(results));
            // console.log(`Monthly supplier report Record selection Released succesfullly by ${decoded.username} on ${new Date()}`);
        });
    });
});
...
router.post('/productCheckOutMonthly', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        var sql = "SELECT DATE_FORMAT(created_date, '%Y-%m-%d') AS DATE, item AS name, SUM(amountSOld) AS value FROM pharmacy.all_check_out_reports WHERE created_date <= LAST_DAY(curdate()) AND created_date >= date_add(date_add(LAST_DAY(curdate()),interval 1 DAY),interval -1 MONTH) group by DATE_FORMAT(created_date, '%Y-%m-%d'), item ORDER BY DATE_FORMAT(created_date, '%Y-%m-%d') ASC";
        connAttrs.query(sql, function (error, results) {
            if (error || results.length < 1) {
                res.set('Content-Type', 'application/json');
                var status = error ? 500 : 404;
...
router.post('/userPerfomanceDay', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        var sql = "SELECT DISTINCT createdBy AS name, SUM(amountSOld) value FROM pharmacy.all_check_out_reports WHERE DATE(created_date) = curdate() GROUP BY createdBy";
        connAttrs.query(sql, function (error, results) {
            if (error || results.length < 1) {
                res.set('Content-Type', 'application/json');
                var status = error ? 500 : 404;
            ...
            res.contentType('application/json').status(200).send(JSON.stringify(results));
            // console.log(`UserPerformance in a day Report selection Released succesfullly by ${decoded.username} on ${new Date()}`);
        });
    });
});
...
// total for the date range selected
router.post('/dateRangeTotal', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        var sql = `SELECT SUM(amountSOld) total FROM all_check_out_reports WHERE created_date >= '${req.body.from}' and created_date <='${req.body.to}' ORDER BY created_date DESC`;
        connAttrs.query(sql, function (error, results) {
            if (error || results.length < 1) {
                res.set('Content-Type', 'application/json');
                var status = error ? 500 : 404;
...
router.post('/supplierPerfomanceMonth', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        var sql = "SELECT DISTINCT supplier AS name, SUM(cost_incured) value FROM pharmacy.VW_SUPPLIER_REPORT WHERE created_date <= LAST_DAY(curdate()) AND created_date >= date_add(date_add(LAST_DAY(curdate()),interval 1 DAY),interval -1 MONTH) GROUP BY supplier";
        connAttrs.query(sql, function (error, results) {
            if (error || results.length < 1) {
                res.set('Content-Type', 'application/json');
                var status = error ? 500 : 404;
            ...
            res.contentType('application/json').status(200).send(JSON.stringify(results));
            // console.log(`Supplier cost performance in a month Report selection Released succesfullly by ${decoded.username} on ${new Date()}`);
        });
    });
});
...
router.post('/itemsTopup', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        var sql = "SELECT * FROM runing_outstock_reports";
        connAttrs.query(sql, function (error, results) {
            if (error || results.length < 1) {
                res.set('Content-Type', 'application/json');
                var status = error ? 500 : 404;
...
router.post('/userWeeklyDiscountAdminView', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        var sql = "SELECT * FROM pharmacy.all_check_out_reports_discount WHERE created_date <= adddate(curdate(), INTERVAL 7-DAYOFWEEK(curdate()) DAY) AND created_date >= adddate(curdate(), INTERVAL 1-DAYOFWEEK(curdate()) DAY) ORDER BY created_date DESC";
        connAttrs.query(sql, function (error, results) {
            if (error || results.length < 1) {
                res.set('Content-Type', 'application/json');
                var status = error ? 500 : 404;
...
router.post('/userMonthlyDiscountAdminView', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        var sql = "SELECT  * FROM pharmacy.all_check_out_reports_discount WHERE created_date <= LAST_DAY(curdate()) AND created_date >= date_add(date_add(LAST_DAY(curdate()),interval 1 DAY),interval -1 MONTH) ORDER BY created_date DESC";
        connAttrs.query(sql, function (error, results) {
            if (error || results.length < 1) {
                res.set('Content-Type', 'application/json');
                var status = error ? 500 : 404;
...
router.post('/userAllAdminViewGeneral', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        var sql = "SELECT  * FROM pharmacy.all_check_out_reports ORDER BY created_date DESC";
        connAttrs.query(sql, decoded.username, function (error, results) {
            if (error || results.length < 1) {
                res.set('Content-Type', 'application/json');
                var status = error ? 500 : 404;
                res.status(status).send(JSON.stringify({
                    status: status,
                    message: error ? "Error getting the server" : "No Records found on users",
                    detailed_message: error ? error.message : "Sorry there are no Records Found."
                }));
                return (error);
            }
...
// reports for sales based on date range
router.post('/dateRange', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        var sql = `SELECT * FROM all_check_out_reports WHERE created_date >= '${req.body.from}' and created_date <='${req.body.to}' ORDER BY created_date DESC`;
        connAttrs.query(sql, function (error, results) {
            if (error || results.length < 1) {
                res.set('Content-Type', 'application/json');
                var status = error ? 500 : 404;
...
router.post('/adminDetails', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        var sql = "SELECT  email, username FROM p_users where role='admin' and deleted_yn ='N'";
        connAttrs.query(sql, decoded.username, function (error, results) {
            if (error || results.length < 1) {
                res.set('Content-Type', 'application/json');
                var status = error ? 500 : 404;
...
router.post('/allExpences', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        var sql = "SELECT * FROM p_expences where deleted_yn ='N' order by expence_id desc";
        connAttrs.query(sql, decoded.username, function (error, results) {
            if (error || results.length < 1) {
                res.set('Content-Type', 'application/json');
                var status = error ? 500 : 404;
...
router.post('/monthlyExpences', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        var sql = "SELECT expence_name name, expence_amount value FROM p_expences where created_date <= LAST_DAY(curdate()) AND created_date >= date_add(date_add(LAST_DAY(curdate()),interval 1 DAY),interval -1 MONTH) AND deleted_yn='N' order by expence_id desc";
        connAttrs.query(sql, decoded.username, function (error, results) {
            if (error || results.length < 1) {
                res.set('Content-Type', 'application/json');
                var status = error ? 500 : 404;
...
router.post('/updateExpence', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        if (err) {
            ...
            let sql = "UPDATE p_expences SET expence_name=?, expence_amount=?, details=?, updated_at = ?, updated_by=? WHERE expence_id=? and deleted_yn='N'"
            connAttrs.query(sql, [expenceToUpdate.expence_name, expenceToUpdate.expence_amount, expenceToUpdate.details,
            expenceToUpdate.updated_at, expenceToUpdate.updated_by, expenceToUpdate.expence_id],
                function (error, results) {
                    if (error) {
                        res.set('Content-Type', 'application/json');
                        res.status(500).send(JSON.stringify({
            ...
            // console.log("=========================================Post:/expence update Released=========================")
...
router.post('/deleteExpence', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        if (err) {
            ...
            connAttrs.query(sql, [expenceToDelete.expence_id],
                function (error, results) {
                    if (error) {
                        res.set('Content-Type', 'application/json');
                        res.status(500).send(JSON.stringify({
            ...
            // console.log("=========================================Post:/Expence Deleting Released=========================")
...
router.post('/newInvoice', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        connAttrs.query(sql, {
            ...
            res.contentType('application/json').status(201).send(JSON.stringify(result));
            // console.log(`New Invoice : ${invoice.invoice_number}, has been Added succesfullly by ${decoded.username} on ${new Date()}`);
        });
    });
});
        ...
        connAttrs.query(sql, decoded.username, function (error, results) {
            if (error || results.length < 1) {
                ...
                res.status(status).send(JSON.stringify({
                    status: status,
                    message: error ? "Error getting the server" : "No Monthly Sales found",
                    detailed_message: error ? error.message : "Sorry there are no Invoices found yet."
                }));
                return (error);
            }
...
router.post('/updateInvoice', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        if (err) {
            ...
            let sql = "UPDATE p_invoice SET supplier_contact=?, supplier=?, updated_at = ?, updated_by=? WHERE id=? and deleted_yn='N'"
            connAttrs.query(sql, [invoiceToUpdate.supplier_contact, invoiceToUpdate.supplier,
            invoiceToUpdate.updated_at, invoiceToUpdate.updated_by, invoiceToUpdate.id],
                function (error, results) {
                    if (error) {
                        res.set('Content-Type', 'application/json');
                        res.status(500).send(JSON.stringify({
...
router.post('/deleteInvoice', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        if (err) {
            ...
            connAttrs.query(sql, [invoiceToDelete.id],
                function (error, results) {
                    if (error) {
                        res.set('Content-Type', 'application/json');
                        res.status(500).send(JSON.stringify({
            ...
            // console.log("=========================================Post:/Deleting Released=========================")
...
router.post('/productInvoice', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        var sql = "SELECT B.name, B.category, B.quantity, A.invoice_number, (A.quantity_to- A.quantity_from) as quantity_added, A.cost_incured, A.created_date from p_logs A INNER JOIN vw_all_items B on A.item_id=B.id where A.invoice_number = ?";
        connAttrs.query(sql, req.body.invoice_number, function (error, results) {
            if (error || results.length < 1) {
                ...
                res.status(status).send(JSON.stringify({
                    status: status,
                    message: error ? "Error getting the server" : "No Items or products found",
                    detailed_message: error ? error.message : "Sorry there are no Products set for that particular invoice. Please consider setting up new products"
                }));
                return (error);
            }

            res.contentType('application/json').status(200).send(JSON.stringify(results));
            // console.log(`All Products per invoice selection Released succesfullly by ${decoded.username} on ${new Date()}`);
        });
    });
});
...
// pulling all the Products per invoice
router.post('/smsBalance', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        var sql = "SELECT org_sms_balance FROM p_org WHERE org_id =?";
        connAttrs.query(sql, decoded.org_id, function (error, results) {
            if (error || results.length < 1) {
                ...
                res.status(status).send(JSON.stringify({
                    status: status,
                    message: error ? "Error getting the server" : "Technical Issue has occured",
                    detailed_message: error ? error.message : "Sorry Technical Issues has occured please Contact system Admin (danie@zyptech.co.ke)"
                }));
                return (error);
            }
...
router.post('/sendToCaroCustomers', function (req, res) {
    var dataTosend = {
        to: req.body.to,
        // message: `Thank you for chosing ${req.body.business} as your Health Care Partner. We assure you of our personalized quality health care services all times, Welcome Again. #StaySafe #WearMask`
        //        message: `Thank you for chosing ${req.body.business} as your Health Care Partner. We assure you of our personalized quality 
        //    health care services all times, you have purchased: ${req.body.message}. Total = ${req.body.total} Welcome Again. #StaySafe`
        message: `Thank you for chosing ${req.body.business} you have purchased: ${req.body.message}. Total = ${req.body.total} Welcome Again. #StaySafe`
    }
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        var toDb = {
            template_message: dataTosend.message,         
            created_by: decoded.username,
    ...
    sms.send(dataTosend)
            .then(response => {
                ...
                if (toDb.template_message.length <= 160){                    
                    connAttrs.query("INSERT INTO p_sms SET ? ", {
                        org_id: decoded.org_id,
                        message: toDb.template_message,                        
                        contact: response.SMSMessageData.Recipients[0].number,
...
// pulling all SMS sent this month.
router.post('/smsSentMonth', function (req, res) {
    ...
    jwt.verify(token, config.jwtSecretKey, function (err, decoded) {
        ...
        var sql = "SELECT * from vw_sms_report where created_date <= LAST_DAY(curdate()) AND created_date >= date_add(date_add(LAST_DAY(curdate()),interval 1 DAY),interval -1 MONTH) order by created_date desc ";
        connAttrs.query(sql, req.body.invoice_number, function (error, results) {
            if (error || results.length < 1) {
                res.set('Content-Type', 'application/json');
                var status = error ? 500 : 404;
                res.status(status).send(JSON.stringify({
                    status: status,
                    message: error ? "Error getting the server" : "No Items or reports found",
                    detailed_message: error ? error.message : "Sorry there are no sms reports "
                }));
                return (error);
            }
